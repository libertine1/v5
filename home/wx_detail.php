...
if($_GET['type']=="job"){
	...
	require_once '../source/function_common.php';
	$uidwxkey=getspace($uid);
	$count3 = $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('goodscod')."  WHERE viewuid='$uid' and uid='$viewuid' and status='1'"),0);
	$query3 = $_SGLOBAL['db']->query("SELECT * FROM ".tname('goodscod')."  WHERE viewuid='$uid' and uid='$viewuid' and status='1' order by dateline DESC");
